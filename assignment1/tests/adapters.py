...
def run_linear(
    ...
    linear = Linear(in_features=d_in, out_features=d_out, _weight=weights)
...
def run_transformer_block(
    ...
    """
    from cs336_basics.Module.Transformer import TransformerBlock
    ...
    transformer_block.MHA.q_proj.weight.data.copy_(weights['attn.q_proj.weight'])
    transformer_block.MHA.k_proj.weight.data.copy_(weights['attn.k_proj.weight'])
    transformer_block.MHA.v_proj.weight.data.copy_(weights['attn.v_proj.weight'])
    transformer_block.MHA.out_proj.weight.data.copy_(weights['attn.output_proj.weight'])
...
def run_transformer_lm(
    ...
    # load parameter
    transformer.embedding.weight.data.copy_(weights["token_embeddings.weight"])
    
    for layer_idx in range(num_layers):
        transformer.layers[layer_idx].MHA.q_proj.weight.data.copy_(
            weights[f"layers.{layer_idx}.attn.q_proj.weight"]
        )
        transformer.layers[layer_idx].MHA.k_proj.weight.data.copy_(
            weights[f"layers.{layer_idx}.attn.k_proj.weight"]
        )
        transformer.layers[layer_idx].MHA.v_proj.weight.data.copy_(
            weights[f"layers.{layer_idx}.attn.v_proj.weight"]
        )
        transformer.layers[layer_idx].MHA.out_proj.weight.data.copy_(
            weights[f"layers.{layer_idx}.attn.output_proj.weight"]
        )

        transformer.layers[layer_idx].FFN.w1.weight.data.copy_(
            weights[f"layers.{layer_idx}.ffn.w1.weight"]
        )
        transformer.layers[layer_idx].FFN.w2.weight.data.copy_(
            weights[f"layers.{layer_idx}.ffn.w2.weight"]
        )
        transformer.layers[layer_idx].FFN.w3.weight.data.copy_(
            weights[f"layers.{layer_idx}.ffn.w3.weight"]
        )

        transformer.layers[layer_idx].norm1.weight.data.copy_(
            weights[f"layers.{layer_idx}.ln1.weight"]
        )
        transformer.layers[layer_idx].norm2.weight.data.copy_(
            weights[f"layers.{layer_idx}.ln2.weight"]
        )

    transformer.norm.weight.data.copy_(weights["ln_final.weight"])
    transformer.output_layer.weight.data.copy_(weights["lm_head.weight"])
...
def run_rmsnorm(
    ...
    rms_norm = RMSNorm(d_model=d_model, eps=eps, _weight=weights)
...
def run_silu(in_features: Float[Tensor, " ..."]) -> Float[Tensor, " ..."]:
    ...
    """
    from cs336_basics.Module.SwiGLU import Swish
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    """
    from cs336_basics.Module.utils import softmax
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    from cs336_basics.Module.utils import clip_gradient
...
def run_get_lr_cosine_schedule(
    ...
    """
    from cs336_basics.Trainer.Optimizer import learning_rate_cosine_schedule

    return learning_rate_cosine_schedule(it, max_learning_rate, min_learning_rate, warmup_iters, cosine_cycle_iters)
...
def run_save_checkpoint(
    ...
    """
    from cs336_basics.Module.utils import save_checkpoint
...
def run_load_checkpoint(
    ...
    """
    from cs336_basics.Module.utils import load_checkpoint
